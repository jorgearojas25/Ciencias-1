...
app.controller('controller', function($scope) {
  ...
  function bubbleSort(myArr){
    ...
    //var ecTempBPr = (5 + (7/2)( tamanio(tamanio - 1) - (tamanio / 2)(tamanio - 1)) + 6(tamanio - 1) +3;
    ...
    for (var i = 0; i < tamanio-1; i++) {
      ...
      for (var j = i+1; j < tamanio; j++) {
        ...
        if (myArr[i] > myArr[j]) {
          ...
          myArr[j] = myArr[i];
          myArr[i] = temp;
          contBubble += 3.5; // var temp = myArr[j]; myArr[j] = myArr[i]; myArr[i] = temp;
        }
      }
    }
    return contBubble;
  ...
  // Sobreescribe arr
  function generarArregloPromedio() {
    $scope.data[0].values = [];
    $scope.data[1].values = [];
    $scope.data[2].values = [];
