var app = angular.module('mejorApp', ['nvd3']);
app.controller('controllerM', function($scope) {

  // Ordenamiento burbula simple
  function bubbleSort(myArr){
    var tamanio = myArr.length;
    //var ecTempBM = 5 ( tamanio(tamanio - 1) - (tamanio / 2)(tamanio - 1) ) + 6(tamanio - 1) +3;
    ...
    for (var i = 0; i < tamanio-1; i++) {
      ...
      for (var j = i+1; j < tamanio; j++) {
        ...
        if (myArr[i] > myArr[j]) {
          ...
          myArr[j] = myArr[i];
          myArr[i] = temp;
          contBubble += 0; // var temp = myArr[j]; myArr[j] = myArr[i]; myArr[i] = temp;
        }
      }
    }
    return contBubble;
  ...
  function insertionSort(myArr) {
    //var ecTempIM = 8( (len/2)(len + 1) - len) + 12(len - 1) + 4
